	...
	for (;;) {
		...
		if (sym == '%') {
			...
			if (sym != OCTOKEN_TOKEN) {
				...
				} else if (lex.fToken == "lex") {
					/*
					 *	Parse the lex token. This gives the class name for our
					 *	lexer class. This also optionally parses a file name
					 *	if the name of our lexer class doesn't match. We assume
					 *	the lexer was generated by our OCLex engine.
					 */

					sym = lex.ReadToken();
					if (sym != OCTOKEN_TOKEN) {
						fprintf(stderr,"%s:%d %%lex must be followed by a class name\n",lex.fFileName.c_str(),lex.fTokenLine);

						retVal = false;
						SkipToNextDeclaration(lex);
					} else {
						lexerClass = lex.fToken;
					}

					sym = lex.ReadToken();
					if (sym != OCTOKEN_STRING) {
						lex.PushBackToken();
						lexerHeader = lexerClass + ".h";
					} else {
