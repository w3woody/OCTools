...
- (BOOL)parse
{
	...
	for (;;) {
		...
		NSLog(@"---- %d",++debugN);
		NSLog(@"STACK:");
		for (OCYaccStack *s in self.stack) {
			NSLog(@"    %d %@",s.state,s.text);
		}
		NSLog(@"SYMBOL: %ld\n",a);
		...
		uint16_t action = ActionTable[a + s.state * MAXTOKENS];
		...
		if ((action & 3) == 0) {
			...
			uint16_t statep = self.stack[[self.stack count] - length - 1].state;
